...
A Direcao C — identidade propria intermediaria com modos legados do CS 1.6, equipes ficticias, tipos simetricos, kits predefinidos e profundidade tatica controlada — esta autorizada como hipotese de prototipagem do MVP, mas ainda nao e uma spec aprovada para implementacao.
...
Os payloads comunitarios iniciais de `cs_rio`, `de_sampa`, `fy_pool_day` e `fy_poolparty` foram removidos do repo principal e movidos para o repositorio opcional `modcsbr/community-maps` com manifests, hashes, creditos e status de redistribuicao.
...
Opcionalmente, instale mapas comunitários a partir do release mais recente de `modcsbr/community-maps`:
...
- [Exploracao de gameplay no backlog: equipes, tipos de jogador e kits predefinidos](specs/backlog/teams-player-types-and-predefined-kits.md) (Direcao C autorizada para prototipagem; nao aprovada para implementacao)
- [Politica no backlog: mapas e assets de terceiros](specs/backlog/third-party-maps-and-assets-policy.md) (rascunho)
