...
- Added conservative Support handling presets for both LMG profiles: `controlled`, `baseline`, and `aggressive` remain complete packages, with smaller fire-rate gains and larger control penalties than Assault to avoid high-ammo laser weapons.
...
- Added narrow Marksman handling presets for Bolt-action and Semi-auto weapons: `controlled` trades rhythm for stability, while `aggressive` improves recovery or fire rate at the cost of worse control, without changing damage, zoom, ammunition ceiling, reload, availability, or weapon family.
...
- Added the check magazine UX rule for collected weapons: their inserted-magazine count stays unknown on the HUD until the player completes a server-authoritative check animation or reloads, preventing free information from ground pickups.
- Set the MVP match structure to 20 rounds, halftime side swap after 10 rounds, first team to 11 round wins, and 10–10 draws without overtime while keeping CS 1.6 round-time, freeze-time, and device-timer baselines for the first playtest.
...
- Added narrow pistol handling presets: `controlled`, `baseline`, and `quick` preserve fixed damage, caliber, reload, 45-round ceiling, availability, suppressor state, burst exclusion, and backup-weapon role.
...
- Added Assault handling presets for community-authored weapons: `controlled`, `baseline`, and `aggressive` are validated as complete recoil/fire-rate/accuracy packages, preventing free slider combinations such as maximum fire rate with minimum recoil.
...
- Added Breacher shotgun handling presets for pump-action and semi-automatic variants, preserving fixed damage, pellet behavior, base range, shell-by-shell reload, 40-shell ceiling, and short-range identity while allowing controlled rhythm/control trade-offs.
