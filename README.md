...
Assault chooses one mutually exclusive Rifle or SMG variant, both with pistol, knife, one fragmentation grenade, and two flashbangs. Its initial global catalog contains four mechanical profiles: 7.62 power/AK-47, 5.56 control/unsuppressed M4A1, 9 mm control/MP5, and .45 power/UMP-45. These weapons are mechanical baselines rather than mandatory visual identities. Assault primaries are full-auto only in the MVP; burst, attachable suppressors, FAMAS, Galil, TMP, MAC-10, and P90 remain outside the initial catalog. Knife mechanics are global even when teams replace its presentation, and weapon damage always follows the server-validated profile. A survivor may carry a collected weapon into the next round as a replacement for the same slot; it receives complete magazines only up to its own category ceiling, and changing type or variant restores the selected predefined kit.

Support uses one MVP kit: machine gun with a 250-round ceiling, pistol with a 45-round ceiling, knife, one flashbang, and one smoke. Its initial damage/reload baseline follows the M249, including a 4.7-second reload. “Suppression” is tactical language for sustained fire inspired by real combat use; in the MVP it remains emergent rather than an artificial opponent penalty.
